    ...
    
{
  ...
  switch (rval = rgetline(stdin, line ,MAX_MENU_RESPONSE_LENGTH)) {
    case AT_EOF :
      return(MENU_EOF);
      break;
    case TOO_MANY_CHARS :
...

{
  ...
  rval = menu_match (
   ...
   // printf("RVAL I____________________ %d %s \n",rval,*ptr_response);
...

{
  ...
  // printf("HERE________________ %d , %d %d",low,high, *ptr_ival);
  if (!(check = (low <= high)))
  ...
  // printf("RVAL II____________________ %d %s",rval,response);
  ...
  switch (rval) {
    ...
    case MENU_DATA :
      ...
      // printf("Numstring %s \n",numstring);
      switch (*ptr_ival = str_to_pos_int(numstring,1,8)) {
        case -1:
        case -2:
