...
char * get_new_value ()
{
  char buffer[200];
  int rval;
  switch (rval = rgetline(stdin,buffer,199)) {
    case AT_EOF :
      user_eof();
      break;
    case TOO_MANY_CHARS :
...
void free_entry (Ptr_Rolo_Entry fentry)
{       
  int j;
  
  for (j = 0; j < N_BASIC_FIELDS ; j++) {
      free(get_basic_rolo_field(j,fentry));
  }
  
  /* copy the user-defined fields, if necessary */
  int n=0;
  n = get_n_others(fentry);     

  if (n > 0) {
     for (j = 0; j < n; j++) { 
         free(get_other_field(j,fentry));
     }
  }
  free(fentry->other_fields);
  free(fentry);        
}
//yv
  ...
  switch (rval) {    
    ...
    case MENU_NO_MATCH : 
      ...
      if (findex < N_BASIC_FIELDS - 1) {
         ...
         printf("New value: ");
         if (0 == (newval = get_new_value())) break;
         ...
         if (strlen(newval) == 0) {
            *s='\0';
            char * t = copystr(other);
            free(newval);
            free(get_other_field(findex,entry));
            // printf("FREEE ONE ENTRY HERE ____________________________ %d %d %s\n",findex,get_n_others(entry),t);
            for (j = findex+1; j < get_n_others(entry); j++) {
                set_other_field(j,entry,get_other_field(j,entry));
            }
            set_other_field(findex,entry,t);
            set_n_others(entry,get_n_others(entry));
         }
         else {   
            *s = '\0';
         ...
         updated = 1;
         // printf("other BF UPDATEEEEE \n");
      }
      else {
        loop:
        printf("New field (<name>: <value>): ");
        if (0 == (newfield = get_new_value())) break;
        if (0 == index(newfield,':')) {
